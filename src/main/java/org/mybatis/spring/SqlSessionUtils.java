...
public final class SqlSessionUtils {
  ...
  private static final class SqlSessionSynchronization extends TransactionSynchronizationAdapter {
    ...
    public void beforeCompletion() {
      // Issue #18 Close SqlSession and deregister it now because afterCompletion may be called from a different thread
    ...
    public void afterCompletion(int status) {
      if (this.holderActive) {
        // afterCompletion may have been called from a different thread  so avoid failing if there is nothing in this one
        LOGGER.debug(() -> "Transaction synchronization deregistering SqlSession [" + this.holder.getSqlSession() + "]");
